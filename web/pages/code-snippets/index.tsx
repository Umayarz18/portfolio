    ...
    >
      ...
      >
        <section className="m-5 md:m-0">
          ...
          >
            There&apos;s a lot of code I end up reusing or constantly looking up
            on Google on how to do it. To save my time and maybe yours,
            here&apos;s list of snippets that might help you out.
          </p>
        </section>
        <section className="grid md:grid-cols-2 gap-2 lg:gap-x-4 m-5 md:m-0">
          {snippets.map((snippet) => (
            <a
              ...
              className="flex flex-col justify-start dark:bg-gray-900 
                          ...
                          dark:border-gray-700 self-center  h-full"
            >
              {snippet.tags && (
                <div className="flex flex-wrap">
                  {snippet.tags.map((tag) => (
                    <Tag color={tag.color} title={tag.title} key={tag._id} />
                  ))}
                </div>
              )}
